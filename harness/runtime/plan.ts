...
import { planningReadiness, defaultTasks, loadState, saveState, writeProgressDoc } from "./planning";
import { readJson, repoRoot } from "./shared";

const root = repoRoot();
const readiness = planningReadiness(root);

if (!readiness.productReady || !readiness.architectureReady || readiness.milestones.length === 0) {
