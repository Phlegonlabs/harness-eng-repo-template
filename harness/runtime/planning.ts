...
import type { DiscoveryState, HarnessState, MilestoneRecord, TaskRecord } from "./types";
import { hasPlaceholderContent, readJson, writeJson } from "./shared";

export function planningReadiness(root: string): { productReady: boolean; architectureReady: boolean; milestones: MilestoneRecord[] } {
...
export function milestonesFromProductDoc(productPath: string): MilestoneRecord[] {
  const content = readFileSync(productPath, "utf8");
  const match = content.match(/^## Proposed Milestones\s*([\s\S]+?)(?=^## |\Z)/m);
  if (!match) return [];
...
export function writeProgressDoc(root: string, milestones: MilestoneRecord[], tasks: TaskRecord[]): void {
  const progress = [
    "# Delivery Progress",
    "",
