...
const eligible = state.milestones.filter((milestone) => milestone.status === "planned" && milestone.parallelEligible && !active.has(milestone.id));
console.log(`Eligible milestones: ${eligible.length}`);
eligible.forEach((milestone) => console.log(`  ${milestone.id}: ${milestone.title}`));
...
for (const milestone of eligible.slice(0, max)) {
  ...
  const worktreePath = path.join(root, ".worktrees", milestone.id.toLowerCase());
  const branchExists = runPassthrough("git", ["-C", root, "rev-parse", "--verify", branch], root) === 0;
  if (!branchExists) {
    if (runPassthrough("git", ["-C", root, "worktree", "add", "-b", branch, worktreePath, "HEAD"], root) !== 0) process.exit(1);
  } else if (!existsSync(worktreePath)) {
    if (runPassthrough("git", ["-C", root, "worktree", "add", worktreePath, branch], root) !== 0) process.exit(1);
  }
