...
| `.harness/validations/` | Latest `harness:validate` and `harness:validate:full` run artifacts |
| `.harness/dispatch/` | Provider-neutral sidecar packet artifacts |
| `.harness/dispatch-results/` | Condensed sidecar result artifacts |
| `.harness/merges/` | Merge audit artifacts for milestone branch reconciliation |
...
Task lifecycle state is split across three fields:

- `status`: execution progress for the task itself
- `contractStatus`: contract artifact state (`missing`, `draft`, `approved`)
- `evaluatorStatus`: latest evaluator outcome (`pending`, `passed`, `failed`)

The runtime task status itself is:
...
- `contractStatus` moves independently as the orchestrator writes and approves the task contract before work starts.
...
- the milestone resolves to at least one affected area
- its resolved affected areas do not overlap with active milestones
- its resolved affected areas do not overlap with other milestones selected in the same dispatch batch

The dispatcher records worktree ownership in `.harness/state.json`.

`bun run harness:merge-milestone -- M1` only proceeds when:

- the main worktree is clean
- the milestone worktree is clean
- all milestone tasks are `done`
- the milestone branch contains commits that are not already on the main branch
- `bun run harness:guardian --mode preflight` passes inside the milestone worktree
- `bun run harness:validate` passes inside the milestone worktree

Merge conflicts are not globally forced to `ours`. Only generated runtime surfaces such as `.harness/state.json` and `docs/progress.md` may be auto-reconciled, and every merge writes a merge audit artifact.
